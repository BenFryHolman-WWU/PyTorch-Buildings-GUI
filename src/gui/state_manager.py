"""State display helpers for component and connection lists."""
...
class StateManager:
    ...
    def refresh_component_list(self):
        """Summary: Refresh component list."""
        ...
        for item in self.canvas.scene.items():
            if hasattr(item, "node") and hasattr(item, "label"):
                ...
                for prop_name, prop_value in values.items():
                    ...
                    child.setText(0, f"{prop_name}: {self._format_property_value(prop_value)}")
                if root.text(0) in expanded:
                    root.setExpanded(True)
        self.refresh_connection_list()

    def refresh_connection_list(self):
        """Summary: Refresh connection list."""
