...
# TODO: Offset in addition to time lag
    ...
    def forward(
            self,
            t: Union[float, torch.Tensor] = 0.,  # [s] Current time
            ...
            dt: Union[float, torch.Tensor, None] = None,  # [s] Time step (for analytic/smooth models)
    ) -> torch.Tensor:
        """
    ...
    def _forward_analytic(self, setpoint: torch.Tensor, position: torch.Tensor, dt: Union[float, torch.Tensor]) -> torch.Tensor:
        """
    ...
    def _forward_smooth_approximation(self, setpoint: torch.Tensor, position: torch.Tensor, dt: Union[float, torch.Tensor]) -> torch.Tensor:
        """
        ...
        assert position is not None and dt is not None, "smooth_approximation requires position and dt"
