...
class Envelope(BuildingComponent):
    ...
    def R_internal_matrix(self) -> torch.Tensor:
        ...
        R_matrix = torch.exp(self.R_internal_logits).clone()
    ...
    def ode_rhs(
            self,
            t: Union[float, torch.Tensor],  # [s] Current simulation time
    ...
    def forward(
            ...
            dt: Union[float, torch.Tensor] = 1.0,  # [s] Time step for integration
    ) -> dict:
        """
